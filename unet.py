...
class MyUNet2D(nn.Module):
    def __init__(self, in_channels, num_classes, level_channels=[16, 32, 64]) -> None:
        ...
        self.conv42 = nn.Conv2d(in_channels= level_channels[2], out_channels=level_channels[2]*2, kernel_size=3, padding=1)
        self.bn42 = nn.InstanceNorm2d(num_features=level_channels[2]*2)
        self.relu = nn.ReLU()
        self.upconv4 = nn.ConvTranspose2d(in_channels=level_channels[2]*2, out_channels=level_channels[2]*2, kernel_size=2, stride=2)

        #level 1 up
        self.conv51 = nn.Conv2d(in_channels= level_channels[2]*3, out_channels=level_channels[2], kernel_size=3, padding=1)
        self.bn51 = nn.InstanceNorm2d(num_features=level_channels[2])
        self.conv52 = nn.Conv2d(in_channels= level_channels[2], out_channels=level_channels[2], kernel_size=3, padding=1)
        self.bn52 = nn.InstanceNorm2d(num_features=level_channels[2])
        self.relu = nn.ReLU()
        self.upconv5 = nn.ConvTranspose2d(in_channels=level_channels[2], out_channels=level_channels[2], kernel_size=2, stride=2)

        #level 2 up
        self.conv61 = nn.Conv2d(in_channels= level_channels[1]*3, out_channels=level_channels[1], kernel_size=3, padding=1)
        self.bn61 = nn.InstanceNorm2d(num_features=level_channels[1])
        self.conv62 = nn.Conv2d(in_channels= level_channels[1], out_channels=level_channels[1], kernel_size=3, padding=1)
        self.bn62 = nn.InstanceNorm2d(num_features=level_channels[1])
        self.relu = nn.ReLU()
        self.upconv6 = nn.ConvTranspose2d(in_channels=level_channels[1], out_channels=level_channels[1], kernel_size=2, stride=2)

        #level 3 up
        self.conv71 = nn.Conv2d(in_channels= level_channels[0]*3, out_channels=level_channels[0], kernel_size=3, padding=1)
        self.bn71 = nn.InstanceNorm2d(num_features=level_channels[0])
        self.conv72 = nn.Conv2d(in_channels= level_channels[0], out_channels=level_channels[0], kernel_size=3, padding=1)
        self.bn72 = nn.InstanceNorm2d(num_features=level_channels[0])
        self.relu = nn.ReLU()

        #last
        self.conv8 = nn.Conv2d(in_channels=level_channels[0], out_channels=num_classes, kernel_size=1)
